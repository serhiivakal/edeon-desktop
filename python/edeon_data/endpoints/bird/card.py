...
def run_card(endpoint: str = None) -> None:
    """Compiles the structured DataCard YAML and generates the release manifest for bird acute oral LD50."""
    ...
    # Source 2: EFSA OpenFoodTox
    source_efsa = SourceMetadata(
        name="EFSA OpenFoodTox",
        citation="EFSA's Chemical Hazards Database",
    ...
    inclusion = [
        "In vivo Bird acute oral LD50 toxicity records only",
        "Endpoint = LD50, exposure route = oral",
        "Species pooled: Colinus virginianus, Anas platyrhynchos, Coturnix japonica, Phasianus colchicus, Passer domesticus",
        "Canonical units: mg/kg bw"
    ]
    
    exclusion = [
        "Mixtures, formulations, or records with missing structures",
        "Non-oral exposure routes (dermal, inhalation, aquatic, etc.)",
        "Non-positive numeric LD50 values",
    ...
    not_intended = [
        ...
        "Quantitative avian risk assessment without local validation"
    ]
    ...
    card = DataCard(
        ...
        splits=splits_info,
        known_biases=biases,
        intended_use="Training and benchmarking Tier-1 QSAR bird acute oral LD50 prediction models.",
