...
def run_split(endpoint: str = None) -> None:
    """Creates scaffold, random, and chronological time splits for Skin Sensitization dataset."""
    ...
    # 2. Random Split (70/15/15) - stratified by GHS class label (value_class)
    train_ra, cal_ra, test_ra, meta_ra = random_split(
        df_curated,
        value_col="value_class",
        classification=True,
        ratios=(0.7, 0.15, 0.15),
        seed=42
    )
    
    # 3. Time Split (70/15/15) - typically not available for skin sensitization
    time_res = time_split(
        df_curated,
    ...
    print("Splitting stage completed successfully for Skin Sensitization.")
