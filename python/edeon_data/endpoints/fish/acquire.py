    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            if smiles and "invalid" not in smiles.lower():
                # Compute exact MW in Python using RDKit
...
def run_acquire(endpoint: str = None) -> None:
    """Verifies ECOTOX ASCII directory and downloads CAS-to-SMILES mappings using hybrid NCI CACTUS + PubChem resolver."""
    if not ECOTOX_DIR.exists():
        raise FileNotFoundError(
            f"Expected ECOTOX ASCII directory at {ECOTOX_DIR}. "
            "Please make sure the unzipped folder is placed in the raw/fish directory."
        )

    # 1. Target species numbers
    species_nums = {1, 2, 4, 68, 5156, 21}
    print("Scanning ECOTOX ASCII files for unique target CAS numbers...")
    ...
    print(f"Found {len(target_cas)} unique target CAS numbers.")

    # 2. Query hybrid resolver in parallel to get chemical structures
    mapping_file = RAW_DIR / "cas_to_smiles.json"
    cache = {}
    if mapping_file.exists():
        with open(mapping_file, "r") as f:
            cache = json.load(f)
        print(f"Loaded {len(cache)} cached CAS-to-SMILES mappings.")

    cas_to_query = [c for c in target_cas if c not in cache]

    if cas_to_query:
        print(f"Resolving {len(cas_to_query)} CAS numbers concurrently via hybrid NCI CACTUS + PubChem resolver...")
        # Use a safe worker count to prevent rate limiting
        ...
        print("All target CAS numbers already mapped in cache.")
