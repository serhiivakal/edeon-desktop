...
from edeon_data.endpoints.fish.acquire import get_chemical_properties

RAW_DIR = Path("/home/svakal/Projects/Edeon/data/raw/fish")
ECOTOX_DIR = RAW_DIR / "ecotox_ascii_03_12_2026"
...
def run_curate(endpoint: str = None) -> None:
    """Standardizes and curates the fish acute LC50 dataset."""
    ...
    if not mapping_file.exists():
        raise FileNotFoundError(f"CAS-to-SMILES mapping file not found at {mapping_file}. Run acquire first.")

    with open(mapping_file, "r") as f:
        cas_to_smiles = json.load(f)

    print("Loading ECOTOX data tables...")
    # 1. Species map
    species_df = pd.read_csv(ECOTOX_DIR / "validation/species.txt", sep="|", low_memory=False)
    species_nums = {1, 2, 4, 68, 5156, 21}
    ...
    for chunk in results_iter:
        ...
        if len(chunk) == 0:
            continue
        chunk = chunk[chunk["endpoint"].astype(str).str.strip().str.upper() == "LC50"]
        chunk = chunk[chunk["effect"].astype(str).str.strip().str.upper() == "MOR"]
    ...
    print(f"Loaded {len(df_results)} raw matched results from ECOTOX.")
    ...
    for idx, row in df_results.iterrows():
        ...
        # Resolve raw SMILES from local cache or hybrid API fallback
        smi_raw_mapping = cas_to_smiles.get(cas_str)
        if not smi_raw_mapping:
            # Query hybrid API directly to ensure complete self-healing resolution
            _, smiles_fetched, mw_fetched = get_chemical_properties(cas_str)
            if smiles_fetched:
                smi_raw_mapping = {"smiles": smiles_fetched, "mw": mw_fetched}
            else:
                smi_raw_mapping = {"smiles": None, "mw": None}
            cas_to_smiles[cas_str] = smi_raw_mapping
        ...
        try:
            ...
            lc50_mol_per_l = (val_mg_l / 1000.0) / mw
            value_log = math.log10(lc50_mol_per_l)
        except Exception:
            value_log = math.log10(val_mg_l)
            flags.append("mw_conversion_failed_log_fallback")

        species_num = test_to_species.get(test_id)
        species_name = species_map.get(species_num, "Fish unspecified")
    ...
    # Williams ensemble dataset support (Excel sheet 'fullycleandata')
    williams_path = RAW_DIR / "williams_ensemble.xlsx"
    williams_records = []
    if williams_path.exists():
        print(f"Loading Williams et al. ensemble dataset from {williams_path}...")
        try:
            xl = pd.ExcelFile(williams_path)
            sheet_name = "fullycleandata" if "fullycleandata" in xl.sheet_names else xl.sheet_names[0]
            df_williams = xl.parse(sheet_name)
            
            for idx, row in df_williams.iterrows():
                raw_cas = row.get("casrn")
                if pd.isna(raw_cas):
                    continue
                ...
                # Resolve structure via cache or hybrid lookup
                smi_raw_mapping = cas_to_smiles.get(cas_str)
                if not smi_raw_mapping:
                    _, smiles_fetched, mw_fetched = get_chemical_properties(cas_str)
                    if smiles_fetched:
                ...
                # toxval_numeric in Williams fullycleandata sheet is stored as log10(mg/L)
                val_log10_mg_l = float(row.get("toxval_numeric", 0))
                val_raw = 10 ** val_log10_mg_l
                    
                # Exact MW and pLC50 molar log calculation
                try:
                    std_mol = Chem.MolFromSmiles(smi_canon)
                    mw = Descriptors.ExactMolWt(std_mol)
                    lc50_mol_per_l = (val_raw / 1000.0) / mw
                    value_log = math.log10(lc50_mol_per_l)
                except Exception:
                    value_log = math.log10(val_raw)
                    
                species_name = str(row.get("species_scientific", "Fish unspecified")).strip()
                year = int(row.get("year")) if pd.notna(row.get("year")) else None

                williams_records.append({
                    "inchikey": ikey,
                    "smiles_canonical": smi_canon,
                    "smiles_original": smi_raw,
                    "cas": cas_str,
                    ...
                    "exposure_duration_h": 96.0,
                    "effect": "mortality",
                    "source": "EPA Williams Ensemble",
                    "source_ref": "Williams et al. (2017)",
                    "source_record_id": str(row.get("study_id")),
                    "year_reported": year,
                    "quality_flags": flags
                })
            print(f"Successfully loaded {len(williams_records)} records from Williams ensemble sheet.")
        except Exception as e:
            print("Error loading Williams ensemble dataset:", e)

    # Save updated cache file
    with open(mapping_file, "w") as f:
        json.dump(cas_to_smiles, f, indent=2)

    if not processed_records and not williams_records:
        raise ValueError("No records successfully processed.")

    df_ecotox = pd.DataFrame(processed_records)
    df_williams = pd.DataFrame(williams_records)

    # 4. Deduplicate and Consolidate: Group by InChIKey
    combined_records = []
    all_ikeys = set()
    if not df_ecotox.empty:
        all_ikeys.update(df_ecotox["inchikey"].tolist())
    if not df_williams.empty:
        all_ikeys.update(df_williams["inchikey"].tolist())

    for ikey in all_ikeys:
        ecotox_group = df_ecotox[df_ecotox["inchikey"] == ikey] if not df_ecotox.empty else pd.DataFrame()
        williams_group = df_williams[df_williams["inchikey"] == ikey] if not df_williams.empty else pd.DataFrame()

        # Overlap preference: ECOTOX > Williams
        if not ecotox_group.empty:
            agg_res = aggregate_records(ecotox_group, mode="regression")
            first = ecotox_group.iloc[0]
            n_total = len(ecotox_group) + len(williams_group)
        else:
            agg_res = aggregate_records(williams_group, mode="regression")
            first = williams_group.iloc[0]
            n_total = len(williams_group)
    ...
    # Save stats
    stats = {
        "raw_records": len(df_results) + len(df_williams),
        "after_parse": len(processed_records) + len(williams_records),
        "after_standardisation": len(processed_records) + len(williams_records),
        "after_filter": len(processed_records) + len(williams_records),
        "after_aggregation": len(df_valid),
        "rejection_rate": (len(df_results) + len(df_williams) - len(processed_records) - len(williams_records)) / (len(df_results) + len(df_williams)) if (len(df_results) + len(df_williams)) > 0 else 0.0
    }
