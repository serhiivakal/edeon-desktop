...
def run_card(endpoint: str = None) -> None:
    """Compiles the structured DataCard YAML and generates the release manifest for Soil DT50."""
    endpoint = "soil_dt50"
    ...
    source_envipath = SourceMetadata(
        name="EAWAG-SOIL via enviPath",
        citation="Wicker J, Fenner K, Ellis L, Wackett L, Griebel S (2016). enviPath - The environmental fate pathway database. Nucleic Acids Res. 44(D1):D502-D508.",
    ...
    sources.append(source_envipath)
    ...
    act_info = ActivityMetadata(
        units_canonical="days",
        log_transform="log10_days",
        aggregation="none",
    ...
    inclusion = [
        "Soil degradation half-life (DT50) values",
        "Individual records preserved (no per-compound value aggregation) to capture soil study variations",
    ...
    not_intended = [
        "Direct regulatory compliance submissions without referencing target soil properties",
        "Averaging the one-to-many records into a single point value without accounting for the variance distribution."
    ]
    ...
    card = DataCard(
        ...
        splits=splits_info,
        known_biases=biases,
        intended_use="Training and benchmarking heteroscedastic Tier-1 QSAR soil DT50 prediction models.",
