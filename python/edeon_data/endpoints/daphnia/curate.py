...
def run_curate(endpoint: str = None) -> None:
    """Standardizes and curates the daphnia acute EC50 dataset."""
    ...
    if not mapping_file.exists():
        # Fall back to fish cache if local daphnia cache doesn't exist
    ...
    print("Loading ECOTOX data tables for Daphnia...")
    # 1. Species map
    species_df = pd.read_csv(ECOTOX_DIR / "validation/species.txt", sep="|", low_memory=False)
    species_nums = {5, 8} # 5: Daphnia magna, 8: Daphnia pulex
    ...
    print(f"Loaded {len(df_results)} raw matched Daphnia results from ECOTOX.")
    ...
    for idx, row in df_results.iterrows():
        ...
        species_name = species_map.get(species_num, "Daphnia unspecified")
        species_taxonomy = species_tax_map.get(species_num, None)
        year_reported = test_to_year.get(test_id)

        # Flag secondary species (Daphnia pulex = 8)
        if species_num == 8:
            flags.append("species_secondary")
        ...
        processed_records.append({
            ...
            "effect": "immobilisation" if row["effect"].strip().upper() == "IMM" else "mortality",
    ...
    if not processed_records:
        raise ValueError("No Daphnia records successfully curated.")
    ...
    print(f"Curation complete: {len(df_valid)} unique daphnia records saved.")
