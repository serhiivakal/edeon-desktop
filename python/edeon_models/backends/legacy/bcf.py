...
class BCF_T2(ModelBackend):
    _ENDPOINT = Endpoint.BCF
    ...
    def predict(self, smiles: list[str], conditions: Optional[dict] = None) -> list[Prediction]:
        ...
        for s in smiles:
            try:
                ...
                if logp is None:
                    logp = 2.0
                log_bcf = max(0.0, min(6.0, 0.85 * logp - 0.70))
                value = 10 ** log_bcf
                log_bcf_lower = max(0.0, log_bcf - 1.18)
                log_bcf_upper = min(6.0, log_bcf + 1.18)
                out.append(Prediction(
                    smiles=s,
                    endpoint=self._ENDPOINT.value,
                    value=PredictionValue(kind="numeric", numeric=float(value)),
                    ci_lower=float(10 ** log_bcf_lower),
                    ci_upper=float(10 ** log_bcf_upper),
                    ci_level=0.95,
    ...
    def metadata(self) -> ModelCard:
        return ModelCard(
            model_id=self.model_id(),
            name="Edeon Legacy BCF (LogP-based)",
            version=self._VERSION,
            tier=2,
            endpoint=self._ENDPOINT.value,
            description=(
                "Tier-2 baseline screening estimate of bioconcentration factor (BCF) "
            ...
            known_failure_modes=[
                ...
                "Highly lipophilic compounds where BCF curves flatten out",
            ],
            references=[],
            license="Proprietary",
