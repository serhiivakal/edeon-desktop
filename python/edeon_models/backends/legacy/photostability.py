...
class Photostability_T2(ModelBackend):
    _ENDPOINT = Endpoint.PHOTOSTABILITY_CLASS
    ...
    def predict(self, smiles: list[str], conditions: Optional[dict] = None) -> list[Prediction]:
        ...
        for s in smiles:
            try:
                ...
                rot = props.get("rotatable_bonds", 3)
                if rot is None: rot = 3
    ...
    def metadata(self) -> ModelCard:
        return ModelCard(
            model_id=self.model_id(),
            name="Edeon Legacy Photostability (LogP-based)",
            version=self._VERSION,
            tier=2,
            endpoint=self._ENDPOINT.value,
            description=(
                "Tier-2 baseline screening estimate of UV photostability "
